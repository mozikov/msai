...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp(torch.norm((x_1[:, :, None] - x_2.t()[None, :, :]), dim=1) / (-2 * sigma))  ### YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    hinge = 1 - scores * labels
    #print('hinge uncutted', hinge.shape)
    hinge[hinge < 0] = 0
    hinge = torch.mean(hinge)
    #print('hinge', hinge.data)
    return hinge ### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = torch.mm(k_batch, self.betas) + self.bias
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            scores = torch.mm(K, self.betas) + self.bias
            # print('scores', scores.shape)
            return scores.flatten() ### YOUR CODE HERE
